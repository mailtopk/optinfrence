...
//$ nvcc dynamic_shap_eng_file.cu -o benchmarkv2 -lnvinfer -lnvToolsExt -I/usr/include/x86_64-linux-gun -L/usr/lib/x86_64-linux-gnu
/*
createExecutionContext()
cudaStreamCreate()
setOptimizationProfileAsync()
setInputShape()
getTensorShape()   // from context
compute sizes
cudaMallocHost()
...
float benchmark_streams(ICudaEngine* engine, int num_streams, const char* input_name, const char* output_name) {
    ...
    for (int i = 0; i < num_streams; ++i) {
        ...
        resources[i].context->setOptimizationProfileAsync(0, resources[i].stream);
        nvinfer1::Dims4 inputDims{1, 3, 224, 224};  // batch = 1
        resources[i].context->setInputShape(input_name, inputDims);

        // Real output dims
        auto out_dims = resources[i].context->getTensorShape(output_name);


        // Size compute
        resources[i].in_size = 1 * 3 * 224 * 224;
...
int main() {
    std::string plan_path = "../enginefiles/resnet50fp16.engine";
